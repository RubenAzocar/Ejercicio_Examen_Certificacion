(function () {
    ...
    const BASE_QUESTION_BANK = {
        1: [
            ...
            { type: "single", prompt: "¿Cuál es la sintaxis correcta para aplicar estilos a dispositivos con un ancho máximo de 768px?", options: ["@media only screen and (max-width: 768) {...}", "@media (min-width: 768px) {...}", "@media screen and (width <= 768px) {...}", "@media (max-width: 768px) {...}"], answer: 3, explanation: "max-width define el límite superior para aplicar los estilos." },
            ...
            { type: "single", prompt: "¿Qué comando se utiliza para agregar cambios al área de preparación (staging area) en Git?", options: ["git merge", "git status", "git add", "git push"], answer: 2, explanation: "git add prepara los archivos para el siguiente commit." },
            ...
            { type: "code", prompt: "Crea un contenedor (div) que incluya un encabezado (h1) con el texto 'Bienvenidos'. El contenido debe estar centrado vertical y horizontalmente, con fondo celeste (#87CEEB) and texto blanco.", expectedKeywords: ["div", "h1", "display", "flex", "center", "#87ceeb", "color"], explanation: "Se espera uso de Flexbox o Grid para centrado y estilos de color.", solutionCode: "<div style=\"display: flex; justify-content: center; align-items: center; height: 100vh; background: #87CEEB; color: white;\">\n  <h1>Bienvenidos</h1>\n</div>" },
            { type: "code", prompt: "Crea un botón con la etiqueta 'Enviar' que cambie de color al pasar el cursor, tenga bordes redondeados, ejecute 'handleButton' al hacer click y muestre un tooltip 'clickear'.", expectedKeywords: ["button", "onclick", "handleButton", "hover", "border-radius", "title"], explanation: "Se evalúa uso de atributos de evento y pseudo-clases CSS.", solutionCode: "<button onclick=\"handleButton()\" title=\"clickear\" class=\"btn\">Enviar</button>\n<style>\n.btn {\n  border-radius: 8px;\n  transition: background 0.3s;\n}\n.btn:hover {\n  background: blue;\n}\n</style>" }
        ],
        2: [
            { type: "code", prompt: "Completa la función para calcular 'promedio', 'maxima' y 'minima' de temperaturas. Escribe la lógica dentro del bucle:\nfunction analizar(temperaturas) {\n  let suma=0, minima=temperaturas[0], maxima=temperaturas[0];\n  for(let temp of temperaturas) {\n    suma += temp;\n    // Escribe aquí //\n  }\n  const promedio = suma / temperaturas.length;\n  return { promedio, maxima, minima };\n}", expectedKeywords: ["if", "temp", ">", "maxima", "<", "minima"], explanation: "Lógica básica de comparación y acumulación.", solutionCode: "for(let temp of temperaturas) {\n  suma += temp;\n  if (temp > maxima) maxima = temp;\n  if (temp < minima) minima = temp;\n}" },
            { type: "code", prompt: "Crea una función que reciba un string y devuelva las palabras que coincidan con él (ignorando mayúsculas) desde una fuente externa 'await getWords()'.", expectedKeywords: ["async", "await", "filter", "includes", "toLowerCase"], explanation: "Uso de métodos de array y normalización de texto.", solutionCode: "async function findWords(target) {\n  const words = await getWords();\n  return words.filter(w => w.toLowerCase().includes(target.toLowerCase()));\n}" },
        ...
        3: [
            ...
            { type: "code", prompt: "Escribe una función async que consulte /api/users con fetch y retorne el JSON.", expectedKeywords: ["async", "await", "fetch", "json"], explanation: "Se espera manejo de asincronía básica." },
            { type: "code", prompt: "Implementa debounce(fn, delay) para limitar llamadas frecuentes.", expectedKeywords: ["setTimeout", "clearTimeout", "return"], explanation: "Patrón avanzado de optimización de eventos." },
            { type: "code", prompt: "Crea una función que agrupe un array de objetos por categoría usando reduce.", expectedKeywords: ["reduce", "acc", "categoria"], explanation: "Transformación compleja de datos con reduce." },
        ...
        4: [
            { type: "single", prompt: "¿Cómo eliminarías un registro específico con id 5 en SQL?", options: ["DROP clientes WHERE id=5", "DELETE FROM clientes", "REMOVE FROM clientes WHERE id=5", "DELETE FROM clientes WHERE id=5"], answer: 3, explanation: "DELETE FROM es la sintaxis estándar de DML." },
            { type: "single", prompt: "¿Cómo cambiarías el tipo de dato de la columna 'telefono' a VARCHAR(20) en la tabla 'clientes'?", options: ["ALTER TABLE clientes MODIFY telefono VARCHAR(20)", "MODIFY TABLE clientes", "UPDATE TABLE clientes", "ALTER TABLE clientes ALTER COLUMN telefono TYPE VARCHAR(20)"], answer: 0, explanation: "ALTER TABLE permite modificar la definición de columnas." },
            { type: "single", prompt: "¿Cómo cambiar el nombre de la tabla 'Empleados' a 'Personal'?", options: ["CHANGE TABLE", "ALTER TABLE Empleados RENAME TO Personal", "UPDATE TABLE", "RENAME AS"], answer: 1, explanation: "RENAME TO es la cláusula para renombrar objetos en SQL." },
            ...
            { type: "code", prompt: "Escribe una consulta SQL que devuelva los nombres de los 3 productos más vendidos (por cantidad total).", expectedKeywords: ["SELECT", "FROM", "GROUP BY", "ORDER BY", "DESC", "LIMIT"], explanation: "Uso de agregaciones y ordenamiento." },
            { type: "code", prompt: "Escribe una consulta que obtenga productos con precio > 100 y estado 'activo', O precio < 50 y estado 'inactivo'.", expectedKeywords: ["SELECT", "WHERE", "AND", "OR"], explanation: "Uso de operadores lógicos." },
            { type: "code", prompt: "Usa un CTE (WITH) para calcular el total de ventas por producto y filtrar los que superen las 500 unidades.", expectedKeywords: ["WITH", "AS", "SELECT", "GROUP BY", "WHERE"], explanation: "Consultas avanzadas con expresiones comunes de tabla." },
