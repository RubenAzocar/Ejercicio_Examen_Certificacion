(function () {
    ...
    const BASE_QUESTION_BANK = {
        ...
        4: [
            { type: "code", prompt: "Escribe la consulta para eliminar un registro específico con id 5 en la tabla 'clientes' en SQL.", expectedKeywords: ["DELETE", "FROM", "clientes", "WHERE", "id", "5"], explanation: "DELETE FROM es la sintaxis estándar de DML.", solutionCode: "DELETE FROM clientes WHERE id = 5;" },
            { type: "code", prompt: "Escribe la consulta para cambiar el tipo de dato de la columna 'telefono' a VARCHAR(20) en la tabla 'clientes'.", expectedKeywords: ["ALTER", "TABLE", "clientes", "MODIFY", "telefono", "VARCHAR(20)"], explanation: "ALTER TABLE permite modificar la definición de columnas.", solutionCode: "ALTER TABLE clientes MODIFY telefono VARCHAR(20);" },
            { type: "code", prompt: "Escribe la consulta para cambiar el nombre de la tabla 'Empleados' a 'Personal'.", expectedKeywords: ["ALTER", "TABLE", "Empleados", "RENAME", "TO", "Personal"], explanation: "RENAME TO es la cláusula para renombrar objetos en SQL.", solutionCode: "ALTER TABLE Empleados RENAME TO Personal;" },
            ...
            { type: "code", prompt: "Escribe una consulta SQL que devuelva los nombres de los 3 productos más vendidos (por cantidad total).", expectedKeywords: ["SELECT", "FROM", "GROUP BY", "ORDER BY", "DESC", "LIMIT"], explanation: "Uso de agregaciones y ordenamiento.", solutionCode: "SELECT nombre FROM productos\nGROUP BY nombre\nORDER BY SUM(cantidad) DESC\nLIMIT 3;" },
            { type: "code", prompt: "Escribe una consulta que obtenga productos con precio > 100 y estado 'activo', O precio < 50 y estado 'inactivo'.", expectedKeywords: ["SELECT", "WHERE", "AND", "OR"], explanation: "Uso de operadores lógicos.", solutionCode: "SELECT * FROM productos\nWHERE (precio > 100 AND estado = 'activo')\n   OR (precio < 50 AND estado = 'inactivo');" },
            { type: "code", prompt: "Usa un CTE (WITH) para calcular el total de ventas por producto y filtrar los que superen las 500 unidades.", expectedKeywords: ["WITH", "AS", "SELECT", "GROUP BY", "WHERE"], explanation: "Consultas avanzadas con expresiones comunes de tabla.", solutionCode: "WITH VentasTotales AS (\n  SELECT producto_id, SUM(unidades) as total\n  FROM ventas\n  GROUP BY producto_id\n)\nSELECT * FROM VentasTotales WHERE total > 500;" },
